...
local function visitAST(cur)
    ...
    if func_name == "gc_root_new"  or func_name == "gc_root_new_with_complex_return" then
      ...
      if func_name == "gc_root_new" then
        if (not isPrimitiveType(f.return_t)) then
          local msg = sprint (" error: call gc_root_new() within %s:%d:%s(), return_t=%s, isPrimitiveType=%s\n",
      ...
      elseif func_name == "gc_root_new_with_complex_return" then
        ...
        if (isPrimitiveType(f.return_t)) then
          local msg = sprint (" error: call gc_root_new_with_complex_return() within %s:%d:%s(), return_t=%s, p=%s",
          f.func_fname, f.func_line, f.func_name, f.return_t
          , isPrimitiveType(f.return_t))
          errmsg = errmsg .. msg
        end
  ...
  local childrens = cur:children()
  if #childrens > 0 then
    for _,c in ipairs(childrens) do
      local c_kind =s
      if c then
        c_kind = c:kind()
      else
...
SECTION "visit AST ... begin"
xprint("tu=%s",tu)
visitAST(tu:cursor())
SECTION "visit AST ... end"
if #errmsg > 0 then
  xprint("\n\n...... [%s] has errors...... \n\n%s" ,arg[1], errmsg)
else
  print("")
  xprint("...... [%s] looks good ......", arg[1])
  print("")
end
SECTION "all finish"
os.exit(0)
